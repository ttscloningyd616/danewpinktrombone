...
var TractUI =
{
    ...
    drawTongueControl : function()
    {
        this.ctx.lineCap = "round";
        this.ctx.lineJoin = "round";
        this.ctx.strokeStyle = paleBlue;
        this.ctx.fillStyle = paleBlue;
        this.ctx.globalAlpha = 1.0;
        this.ctx.beginPath();
    ...
    handleTouches : function()
    {
        ...
        if (this.tongueTouch == 0)
        {
            for (var j=0; j<UI.touchesWithMouse.length; j++)
            {
                ...
                if (!touch.alive && !touch.simulated) continue;
        ...
        {
            var touch = UI.touchesWithMouse[j];
            if (!touch.alive && !touch.keepingWithExploreMode && !touch.simulated) continue;
            var x = touch.x;
            var y = touch.y;
            var index = TractUI.getIndex(x,y);
            var diameter = TractUI.getDiameter(x,y);
